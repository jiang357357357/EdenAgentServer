...
function describeConnectorPlugin() {
  return {
    ...
    test: 'Write node:test cases using WorkerFrameReader/encodeWorkerFrame and the execution sandbox. Test initialize/health/query/execute/event/shutdown, partial frames, wrong identity, cancellation and denied resources. Use temporary fixtures; never user Data. Tests and command execution require their normal permissions.',
    ...
    permissions: 'Declare every connector permission also in plugin.json. filesystem.read/write resources must reference settings.<field>; host rewrites approved paths to sandbox mounts. network is a fixed endpoint bridge declared in connector.json. Identity uses environment.read connector.identityCredential. No host environment inheritance. Permission grants are never available to generated code.',
