...
const stateSchema = z.object({ enabled: z.boolean(), next_wake_at: z.string().nullable().optional(),
  next_wake_reason: z.string().nullable().optional(), next_wake_source: z.string().nullable().optional(), last_error: z.string().nullable().optional(),
...
export function readExternalSchedule(filename?: string) {
  ...
  try {
    ...
    return { status: 'scheduled' as const, nextWakeAt: date.toISOString(), source: state.next_wake_source ?? 'unknown', reason: state.next_wake_reason ?? 'MonOs 自醒计划' }
