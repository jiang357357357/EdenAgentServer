...
export function directSkillTools(service: SkillService, executor: RuntimeTool, profile: string): RuntimeTool[] {
  ...
  return service.repository.list(false).filter(skill => skill.enabled && skill.modelInvocable && supportsSkillProfile(skill.profiles, profile))
    .flatMap(skill => (skill.codeTools ?? []).map((tool): RuntimeTool => ({
      identity: `skill:${skill.name}:${tool.name}`, source: 'skill',
      name: tool.name, description: tool.description, parameters: tool.parameters,
      revision: `skill:${skill.name}:${skill.contentHash}:${skill.workspaceRoot}`, executionMode: 'sequential' as const,
      execute(raw, context) {
        service.repository.read(skill.name, false, { expectedContentHash: skill.contentHash, expectedWorkspaceRoot: skill.workspaceRoot })
        return executor.execute({ name: skill.name, tool: tool.name, arguments: raw }, context)
      },
    })))
}
