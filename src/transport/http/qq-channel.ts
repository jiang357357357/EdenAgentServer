...
export class QqChannelHttp {
  private readonly tasks = new Map<Promise<void>, IncomingMessage>()
  private closed = false
  constructor(private readonly origin: 'mon' | 'local', private readonly bridge?: QqChannelBridge) {}

  handle(request: IncomingMessage, response: ServerResponse): boolean {
    if (!paths.includes(request.url ?? '')) return false
    if (this.origin !== 'mon') { reply(response, 404, { error: 'Not found' }); return true }
    if (this.closed || !this.bridge) { reply(response, 503, { error: 'QQ service identity unavailable' }); return true }
    if (request.method !== 'POST' || request.headers.origin) { reply(response, 403, { error: 'Service requests only' }); return true }
    if (this.tasks.size >= 16) { reply(response, 429, { error: 'QQ request limit reached' }); return true }
    const task = this.dispatch(request, response).catch(error => {
      reply(response, error instanceof BodyError ? error.status : 400,
        { error: error instanceof Error ? error.message : 'QQ request could not be processed' })
    })
  ...
  private authorize(request: IncomingMessage, body: Buffer): boolean {
    ...
    const timestamp = header('x-mon-service-timestamp'), nonce = header('x-mon-service-nonce')
    const signature = header('x-mon-service-signature')
    if (header('x-mon-service-id') !== 'moncore' || header('x-mon-service-scope') !== 'qq:submit') return false
    ...
    const expected = serviceSignature(this.bridge!.identity.secret, 'moncore', 'qq:submit', timestamp, nonce, request.url!, body)
    if (!timingSafeEqual(Buffer.from(signature, 'hex'), Buffer.from(expected, 'hex'))) return false
    try { this.bridge!.consumeNonce(nonce); return true } catch { return false }
  }
}
