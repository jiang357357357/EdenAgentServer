...
from .connectors import create_connector_tools
...
def create_mon_agent_tools(
    ...
    tools.extend(create_connector_tools(context))
    ...
    for tool in tools:
        if tool.name not in allowed:
            continue
        source = "coding" if tool.name in coding_tools else "connector" if tool.name in CONNECTOR_TOOL_NAMES else "builtin"
        registry.register(finalize_tool(tool, source=source), source=source)
