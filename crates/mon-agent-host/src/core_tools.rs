...
use mon_agent_domain::{BlobId, SessionId, TurnId};
    ...
    if matches!(channel, "email" | "both") || (channel == "auto" && delivered.is_empty()) {
        ...
        match core
            .request(
                ...
                "/api/agent/external-email/send/",
                Some(json!({
                    "subject":if title.is_empty() { "MonAgent 提醒" } else { title },
                    "content":text,
                    "metadata":metadata,
...
mod tests {
    ...
    async fn fake_host() -> (HostServices, CapturedRequests, tokio::task::JoinHandle<()>) {
        ...
        let store = mon_agent_store::Store::in_memory().await.expect("store");
    ...
    async fn real_core_sticker_round_trip_cleans_up_its_artifact() {
        ...
        let session = host
            ...
            .await
            .expect("session");
        let name = format!("MonAgent迁移验收临时贴纸-{}", now_millis());
            ...
            .execute(
                &ToolCall {
                    ...
                    arguments: json!({
                        "imageUrl":image_path,
                        "name":name,
                        "description":"MonAgent 全 Rust 迁移验收临时图片",
                        "emotion":"测试",
                        "intent":"验证贴纸创建发送删除链路",
                        "aliases":["MonAgent迁移验收临时别名"]
                    }),
                },
                context(Some(session.id), Some(TurnId::new()), json!({})),
            )
    ...
    async fn real_core_email_delivery_is_confirmed_by_the_transport() {
        ...
        let request_id = format!("monagent-real-email-{}", now_millis());
        let host = HostServices::new(
        ...
        .execute(
            &ToolCall {
                ...
                arguments: json!({
                    "subject":"MonAgent 全 Rust 迁移验收",
                    "content":format!("这是一封 MonAgent 自动化迁移验收邮件。request_id={request_id}"),
                    "to":[recipient],
    ...
    async fn real_core_qq_delivery_requires_a_confirmed_bot_ack() {
        ...
        let request_id = format!("monagent-real-qq-{}", now_millis());
        let host = HostServices::new(
            mon_agent_store::Store::in_memory().await.expect("store"),
            Some(&base),
            Some(&token),
        ...
        .execute(
            &ToolCall {
                ...
                arguments: json!({
                    ...
                    "targetType":target_type,
                    "targetQqNumber":target,
                    "content":format!("MonAgent 全 Rust 迁移自动验收消息（{request_id}）"),
