...
impl WebTool {
    async fn search(
        ...
        let deadline = Instant::now() + total_timeout("MON_AGENT_SEARCH_TOTAL_TIMEOUT_MS", 30_000);
    ...
    async fn open(
        ...
        let deadline = Instant::now() + total_timeout("MON_AGENT_FETCH_TOTAL_TIMEOUT_MS", 30_000);
...
fn provider_request(
    ...
    match provider {
        ...
        "searxng" => {
            let mut url = provider_url(&["MON_AGENT_SEARXNG_URL", "SEARXNG_URL"], None)?;
...
fn provider_order(requested: Option<&str>) -> Result<Vec<String>, ToolFailure> {
    let configured = requested
        ...
        .map(str::to_owned)
        .or_else(|| env::var("MON_AGENT_SEARCH_PROVIDER").ok())
    ...
    if configured.trim().eq_ignore_ascii_case("auto") {
        ...
        if has_env(&["MON_AGENT_SEARXNG_URL", "SEARXNG_URL"]) {
...
fn provider_timeout() -> Duration {
    Duration::from_millis(env_u64("MON_AGENT_SEARCH_TIMEOUT_MS", 10_000).clamp(1_000, 60_000))
}

fn cache_ttl() -> Duration {
    Duration::from_secs(env_u64("MON_AGENT_SEARCH_CACHE_TTL_SECONDS", 120).min(3_600))
}

fn fetch_max_bytes() -> usize {
    usize::try_from(env_u64("MON_AGENT_FETCH_MAX_BYTES", 1_000_000).clamp(64_000, 8_000_000))
        .unwrap_or(1_000_000)
}
...
mod tests {
    ...
    fn structured_results_are_ranked_deduplicated_and_host_limited() {
        let results = normalize_results(
            ...
            vec![
                ...
                json!({"title":"MonAgent Rust guide","url":"https://one.example/guide#part","description":"exact"}),
                json!({"title":"MonAgent Rust guide","url":"https://mirror.example/duplicate"}),
    ...
    async fn find_uses_session_page_references_with_unicode_safe_offsets() {
        let host = HostServices::new(
            mon_agent_store::Store::in_memory().await.expect("store"),
            None,
            None,
        )
        .expect("host");
