//! Durable, user/session-scoped projection of MonAgent state into Mon Core.
...
impl CoreSyncService {
    pub fn new(store: Store) -> Result<Self, CoreSyncError> {
        let client = Client::builder()
            ...
            .user_agent(format!("MonAgent-CoreSync/{}", env!("CARGO_PKG_VERSION")))
            .build()
...
async fn director_projection(
    ...
    Ok(json!({
        "session":{
            "source":"monagent","external_session_id":session.id,"assistant":assistant_id,
