...
use mon_agent_store::{EventRecord, MemoryRecord};
...
fn identity_section(
    ...
    let name = character
        ...
        .unwrap_or_else(|| "MonAgent".to_owned());
    let mut lines = if name == "MonAgent" {
        vec![
            "你是 MonAgent，一个运行在 Mon 项目中的本地智能体。".to_owned(),
        ...
        vec![
            ...
            "持续使用该角色的姓名、关系和表达方式，对本轮判断、行动与回复负责。不要自称 MonAgent，除非角色资料明确要求。".to_owned(),
        ]
    };
...
pub(crate) fn runtime_environment_context(environment: &Value) -> String {
    let time = mon_agent_environment::current_time_context(environment);
    let timezone = environment
        .get("timezone")
        .and_then(Value::as_str)
        .filter(|value| !value.trim().is_empty())
