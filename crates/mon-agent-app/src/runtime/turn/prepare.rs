...
use mon_agent_core::{AgentContext, AgentError, Message, ModelSpec};
use mon_agent_store::{InputRecord, StoreError};
...
pub(super) struct PreparedTurn {
    ...
    pub(super) session_events: Vec<mon_agent_store::EventRecord>,
...
pub(super) async fn prepare_turn(
    ...
    let runtime_participants = if session.participants.is_empty() {
        vec![json!({
            ...
            "assistantName":"MonAgent",
            "characterId":"runtime-default",
            "characterName":"MonAgent",
        })]
    } else {
