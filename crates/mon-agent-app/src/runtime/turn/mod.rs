...
use mon_agent_core::{AgentError, LoopControl};
use mon_agent_store::InputRecord;
use std::sync::Arc;
use tokio_util::sync::CancellationToken;

pub(super) type SelfAwakeState = (
    mon_agent_store::JobRecord,
    mon_agent_store::SelfAwakeRunRecord,
...
pub(in crate::runtime) async fn process_input(
    ...
    let run_result = if prompt_profile == prompt::PromptProfile::SelfAwake {
        let timeout_seconds = std::env::var("MON_AGENT_SELF_AWAKE_TIMEOUT_SECONDS")
            .ok()
