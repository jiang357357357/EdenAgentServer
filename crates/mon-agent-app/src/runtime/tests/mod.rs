...
use mon_agent_store::{EventRecord, Store};
...
#[async_trait]
impl mon_agent_core::Tool for NamedTool {
    ...
    async fn execute(
        &self,
        _call: &mon_agent_core::ToolCall,
        _context: mon_agent_core::ToolCallContext,
    ) -> Result<mon_agent_core::ToolOutput, mon_agent_core::ToolFailure> {
        Ok(mon_agent_core::ToolOutput::text("ok"))
    }
}

struct ReloadableUnsafeTool;

impl mon_agent_core::DynamicToolSource for ReloadableUnsafeTool {
    fn get(&self, name: &str) -> Option<Arc<dyn mon_agent_core::Tool>> {
        match name {
