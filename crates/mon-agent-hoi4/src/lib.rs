...
pub const BRIDGE_MARKER: &str = "MONAGENT_HOI4|";
...
mod tests {
    ...
    fn parses_prefixed_snapshot_into_typed_state() {
        let record = parse_bridge_record(
            "[12:00:00][effectbase.cpp:123]: MONAGENT_HOI4|1|SNAPSHOT|date=1939.9.1|country_tag=GER|country_name=German Reich|political_power=125.50|stability=0.72|civilian_factories=31|at_war=yes",
        )
    ...
    fn ignores_unknown_protocols_and_unrelated_logs() {
        ...
        assert!(parse_bridge_record("MONAGENT_HOI4|2|HELLO|bridge_version=2").is_none());
        assert!(parse_bridge_record("[MONAGENT]|1|SNAPSHOT|country_tag=ENG").is_none());
    }
    ...
    async fn follows_log_rotation_and_updates_latest_snapshot() {
        ...
        let mut file = fs::OpenOptions::new()
            ...
            .expect("open log");
        file.write_all(
            b"MONAGENT_HOI4|1|HELLO|bridge_version=0.1.0|mode=observe\nMONAGENT_HOI4|1|SNAPSHOT|date=1936.1.1|country_tag=ENG|political_power=50|at_war=no\n",
        )
        .await
        ...
        let replacement = format!(
            "MONAGENT_HOI4|1|HELLO|bridge_version=0.1.0|mode=observe\nMONAGENT_HOI4|1|SNAPSHOT|date=1936.2.1|country_tag=FRA|political_power=60\n{}\n",
