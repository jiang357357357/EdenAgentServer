...
impl SkillCatalog {
    ...
    pub fn install_generated_package(
        ...
        let loaded: LoadedCatalog =
            serde_json::from_value(mon_agent_tools::load_skills(&[staged_directory
                .to_string_lossy()
                .into_owned()]))?;
    ...
    pub fn inspect_local_for(
        ...
        let loaded: LoadedCatalog =
            serde_json::from_value(mon_agent_tools::load_skills(&[target
                .to_string_lossy()
                .into_owned()]))?;
        let skill =
    ...
    pub fn install_preview_for(
        ...
        fs::write(
            staged.join(".monagent-install.json"),
    ...
    pub fn preview_generated_update(
        ...
        let preview_root = tempfile::Builder::new()
            .prefix("monagent-skill-update-preview-")
            .tempdir()?
            .keep();
...
fn load_one_skill(directory: &Path, install_root: &Path) -> Result<SkillDefinition, SkillError> {
    let loaded: LoadedCatalog = serde_json::from_value(mon_agent_tools::load_skills(&[directory
        .to_string_lossy()
        .into_owned()]))?;
    let skill =
        loaded.skills.into_iter().next().ok_or_else(|| {
...
fn enrich_skill(
    ...
    let monagent = frontmatter
        .get("metadata")
        .and_then(|value| value.get("monagent"))
        .cloned()
        .unwrap_or_else(|| json!({}));
    let string = |name: &str| {
        monagent
...
impl SkillCodeTool {
    fn tool_definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            exposure: mon_agent_core::ToolExposure::Direct,
        }
    }
    ...
    ) -> Result<mon_agent_tools::SandboxedProgramOutput, ToolFailure> {
...
impl Tool for CreateSkillTool {
    ...
    fn permission_request(&self, arguments: &Value) -> Option<mon_agent_core::PermissionRequest> {
        Some(mon_agent_core::PermissionRequest {
...
impl Tool for UpdateSkillTool {
    ...
    fn permission_request(&self, arguments: &Value) -> Option<mon_agent_core::PermissionRequest> {
        (arguments.get("action").and_then(Value::as_str) == Some("apply")).then(|| {
            mon_agent_core::PermissionRequest {
                permission: "skill.write".to_owned(),
                patterns: vec![
                    arguments
                        .get("name")
...
mod tests {
    ...
    async fn code_tools_are_sandbox_gated_executable_and_reloadable() {
        ...
        let mut registry = mon_agent_core::ToolRegistry::new();
        ...
        let (events, _receiver) = mon_agent_core::event_channel(4);
        let output = tool
            .execute(
                &ToolCall {
                    id: "call-1".to_owned(),
        ...
        let (events, _receiver) = mon_agent_core::event_channel(1);
        let error = tool
            .execute(
                &ToolCall {
