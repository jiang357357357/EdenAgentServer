...
pub const BRIDGE_MARKER: &str = "[MONAGENT]|";
...
mod tests {
    ...
    fn parses_prefixed_bridge_records() {
        let record = parse_bridge_record(
            "[12:00:00][jomini_log.cpp:35]: [MONAGENT]|1|SNAPSHOT|date=1840.1.1|country_id=42|country_name=Great Qing",
        )
    ...
    fn ignores_unknown_protocols_and_unrelated_logs() {
        ...
        assert!(parse_bridge_record("[MONAGENT]|2|HELLO|bridge_version=2").is_none());
        assert!(parse_bridge_record("[MONAGENT]|1|ACK|status=success").is_none());
    }

    #[test]
    fn parses_control_ack() {
        let record = parse_bridge_record(
            "[MONAGENT]|1|ACK|command_id=018f-test|status=success|action=probe_control",
        )
        .expect("ack record");
    ...
    async fn follows_log_and_updates_snapshot_state() {
        ...
        let mut file = fs::OpenOptions::new()
            ...
            .expect("open log");
        file.write_all(
            b"[MONAGENT]|1|HELLO|bridge_version=0.1.0\n[MONAGENT]|1|SNAPSHOT|date=1842.3.15|country_id=CHI|gold_reserves=125000\n",
        )
        .await
        ...
        file.write_all(
            b"[MONAGENT]|1|ACK|command_id=018f-test|status=success|action=probe_control\n",
        )
        .await
        .expect("append ack");
        assert!(matches!(
