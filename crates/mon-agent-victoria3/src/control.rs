...
impl Controller {
    ...
    pub async fn probe(&self) -> Result<ControlProbeResult, String> {
        ...
        let stem = format!("monagent_{}", command_id.replace('-', ""));
...
fn render_probe_effect(command_id: &str) -> String {
    format!(
        "debug_log = \"[MONAGENT]|{PROTOCOL_VERSION}|ACK|command_id={command_id}|status=success|action=probe_control\"\n"
    )
}
...
mod tests {
    ...
    fn renders_only_the_ack_probe_effect() {
        ...
        assert_eq!(
            effect,
            "debug_log = \"[MONAGENT]|1|ACK|command_id=018f-test|status=success|action=probe_control\"\n"
        );
