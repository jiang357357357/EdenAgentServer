use mon_agent_core::{ModelError, ModelSpec};
...
impl OpenAiCompatibleConfig {
    pub fn from_env() -> Result<Self, ModelError> {
        ...
        let base_url = std::env::var("MON_AGENT_BASE_URL")
...
pub fn model_spec_from_env() -> ModelSpec {
    let raw_model =
        std::env::var("MON_AGENT_MODEL").unwrap_or_else(|_| "openai/gpt-4o-mini".to_owned());
    ...
    extra.insert(
        ...
        Value::Bool(env_bool("MON_AGENT_MODEL_SUPPORTS_IMAGES", true)),
    );
    ModelSpec {
        id: model_id.to_owned(),
        provider: provider.to_owned(),
        api: "openai-completions".to_owned(),
        base_url: std::env::var("MON_AGENT_BASE_URL")
            .or_else(|_| std::env::var("OPENAI_BASE_URL"))
            .ok(),
        context_window: Some(env_u32("MON_AGENT_CONTEXT_WINDOW", 128_000) as u64),
        max_tokens: Some(env_u32("MON_AGENT_MAX_OUTPUT_TOKENS", 16_384) as u64),
        extra,
    }
}
