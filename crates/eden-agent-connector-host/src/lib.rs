...
use eden_agent_connector_package::{LoadedPackage, PackageError};
...
pub struct WorkerProcess {
    client: WorkerClient,
    notifications: mpsc::Receiver<eden_agent_connector_protocol::RpcNotification>,
...
impl WorkerProcess {
    pub async fn launch(config: WorkerLaunchConfig) -> Result<Self, WorkerError> {
        ...
        if initialization.protocol_version != eden_agent_connector_protocol::PROTOCOL_VERSION {
            cancellation.cancel();
            return Err(WorkerError::ProtocolVersion {
                expected: eden_agent_connector_protocol::PROTOCOL_VERSION,
    ...
    pub async fn recv_notification(
        &mut self,
    ) -> Option<eden_agent_connector_protocol::RpcNotification> {
...
pub enum WorkerError {
    ...
    Remote(#[from] eden_agent_connector_protocol::RpcError),
...
mod tests {
    use super::*;
    use eden_agent_connector_package::{ConnectorPackageManifest, IntegrityState};
