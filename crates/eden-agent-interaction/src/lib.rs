...
use eden_agent_blob::{BlobError, BlobService};
use eden_agent_core::{Tool, ToolCall, ToolCallContext, ToolDefinition, ToolFailure, ToolOutput};
use eden_agent_domain::{QuestionRequestId, SessionId, TurnId};
use eden_agent_store::{MediaRequestRecord, QuestionRecord, Store, StoreError};
...
impl Tool for MediaCaptureTool {
    ...
    async fn execute(
        ...
        Ok(ToolOutput {
            content: vec![
                eden_agent_core::ContentBlock::Text {
                    text: format!("Captured {} image", self.kind),
                },
                eden_agent_core::ContentBlock::Image {
...
mod tests {
    use super::*;
    use eden_agent_core::{ContentBlock, ToolCall, ToolCallContext, event_channel};
