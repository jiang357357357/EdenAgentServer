use eden_agent_connector_package::{LoadedPackage, PackageCatalog};
use regex::Regex;
...
mod tests {
    ...
    fn enforces_declared_string_numeric_pattern_and_uuid_constraints() {
        ...
        for invalid in [
            ...
            json!({"port":40092,"name":"EDENAGENT","session":"018f3f43-7b8a-7fd2-9123-123456789abc"}),
    ...
    fn plugin_connector_packages_are_atomic_and_collision_checked() {
        ...
        let write_package = |id: &str| {
            ...
            LoadedPackage::load(
                package_root.path(),
                eden_agent_connector_package::LoadPolicy::Development,
            )
    ...
    fn identical_plugin_bundle_can_shadow_its_standalone_compatibility_package() {
        ...
        fs::write(
            ...
            serde_json::to_vec(&json!({
                ...
                "entrypoints":{eden_agent_connector_package::current_platform():{"path":"worker","args":[]}},
                "settingsSchema":{"type":"object","additionalProperties":false},
                "events":{},"queries":{},"actions":{}
            }))
            .expect("manifest"),
