...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let output = env::args().nth(1).map_or_else(
        || PathBuf::from("frontend/web/src/generated/mon-agent-rpc.ts"),
        PathBuf::from,
    );
...
export const MON_AGENT_PROTOCOL_VERSION = {protocol_version} as const
export const MON_AGENT_WEBSOCKET_PROTOCOL = {websocket_protocol:?} as const
export const MON_AGENT_TOKEN_PROTOCOL_PREFIX = {token_prefix:?} as const
...
"#,
        protocol_version = mon_agent_api::PROTOCOL_VERSION,
        websocket_protocol = mon_agent_api::WEBSOCKET_PROTOCOL,
        token_prefix = mon_agent_api::TOKEN_PROTOCOL_PREFIX,
    );
