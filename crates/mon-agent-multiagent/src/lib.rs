...
use mon_agent_domain::{AgentId, SessionId, TurnId};
use mon_agent_store::{AgentThreadRecord, Store, StoreError};
...
async fn run_agent(inner: Arc<Inner>, record: AgentThreadRecord) -> Result<(), MultiAgentError> {
    ...
    let execution = async {
        if terminal_checkpoint {
            return Ok(mon_agent_core::RunResult {
...
mod tests {
    ...
    fn install_bounded_role(root: &TempDir, max_tokens: u64) {
        let agents = root.path().join(".monagent/agents");
