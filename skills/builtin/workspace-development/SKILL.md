...
metadata:
  edenagent:
    ...
    profiles: [user_chat, subagent]
---

- Use `switch_workspace` only after the user explicitly asks to change the active project. The switch is durable and applies after all current agent work and command sessions become idle.
- For paths outside the active workspace, use only `external_ls`, `external_find`, `external_read`, or `external_grep`; each call requires an explicit approved root and is read-only and bounded.
